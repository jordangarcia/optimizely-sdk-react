...
import { OptimizelyFeatureVariables } from './optimizely-sdk-react/FeatureVariables'
...
class App extends React.Component<AppProps> {
  ...
  render() {
    ...
    return <OptimizelyProvider datafile={datafile} userId="jordan">
        <div className="App">
          ...
          <Example title="Experiment Example 2">
            <OptimizelyExperimentSwitch experiment="abtest1">
              ...
              <OptimizelyMatch default render={() => <h1>
                    variation 2 (default)
                  </h1>} />
            </OptimizelyExperimentSwitch>
          </Example>
          ...
          <Example title="Feature example">
            <OptimizelyFeature feature="feature1" renderEnabled={(featureVariables: VariableValuesObject) => <div>
                  "feature1" is enabled
                  <pre>{JSON.stringify(featureVariables, null, '  ')}</pre>
                </div>} renderDisabled={() => `Feature1 is disabled`} />
          </Example>

          <Example title="FeatureVariableSwitch example">
            <OptimizelyFeatureVariableSwitch feature="feature1" variable="header">
              <OptimizelyMatch value="Hi Jess!" render={variableValues => <p>
          ...
          <Example title="FeatureVariableSwitch with matcher example">
            <OptimizelyFeatureVariableSwitch feature="feature1" variable="variation" matcher={(variableVal, matchVal) => (matchVal as Array<string>).indexOf(
                  variableVal as string,
                ) > -1}>
              <OptimizelyMatch value={['jordan', 'jess']} render={variableValues => <p>
                    (1) Got variation {variableValues.variation}
                  </p>} />
              <OptimizelyMatch value={['jess']} render={variableValues => <p>
                    (2) Got variation {variableValues.variation}
                  </p>} />
              <OptimizelyMatch default render={variableValues => <p>
                    (default) Got variation {variableValues.variation}
                  </p>} />
            </OptimizelyFeatureVariableSwitch>
          </Example>

          <Example title="FeatureVariables (render prop) example">
            <OptimizelyFeatureVariables feature="feature1" render={variables => <pre>
